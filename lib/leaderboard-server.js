...
export async function getRankedScores(sql, date) {
  ...
  if (date === istDate()) {
    ...
    rows = archived || await sql`select id, player_name as username, player_name_key as "userId", score, hits, attempts, accuracy, created_at as "playedAt" from scores where (created_at at time zone 'Asia/Kolkata')::date = ${date}::date order by score desc, created_at asc`;
  }
