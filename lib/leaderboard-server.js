...
export async function getLeaderboardDates(sql) {
  ...
  rows.forEach((row) => { const value = String(row.date); const totalPlayed = Number(row.total_played) || 0; const existing = merged.get(value); if (!existing || totalPlayed > existing.totalPlayed) merged.set(value, { value, totalPlayed }); });
